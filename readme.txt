...
The new event will be shown as an integration setting, and if enabled, anytime a user is logged in, you'll receive the notification in Slack.

**Development of this plugin is done on [GitHub](https://github.com/cleancoded/wordpress-slack-notifications). Pull requests are always welcome**.
...
1. Add new **Incoming WebHooks** service in your Slack, the URL is `https://<SUBDOMAIN>.slack.com/services/new/incoming-webhook` (replace `<SUBDOMAIN>` with your Slack's subdomain). Once created, note the URL of the service (you'll set it into integration entry in your WordPress).
